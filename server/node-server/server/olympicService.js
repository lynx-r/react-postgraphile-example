...
const connection = new Client({host: 'localhost', user: 'react', password: 'react', database: 'olympic_winners'});
connection.connect();
...
const select = args => {
  if (args.rowGroups.length > 0) {
    ...
    console.log('groupsToUse', groupsToUse);
...
const from = () => ' FROM public.olympic_winners';
...
const limit = (args) => {
  if (args.endRow) {
    ...
    return ` OFFSET ${args.startRow} LIMIT ${pageSize + 1}`;
  }
  return '';
};
